			...
			success: function(data) {
				for (i=0;i<data.length;i++){
						...
						for(j = 0; j < studId.length ; j ++){
							if(sid == studId[j]){
								remark = 0;
								send_sms(data[i]['sid'], global_subject);
							}
							else{
								remark = 1;
							}
...
function add_attendance(id, sub, remark){
		$.ajax({  
			type: 'POST',  
			url: 'queries.php', 
			data: {add_attendance: 1, e_id: id, sub_id2: sub, remark : remark, attdate : $('#attdate').val()  },
			success: function(data) {
...
var stud_array = [];
function sel_subject_section(){
		var counter = 1;
		$.ajax({  
			type: 'POST',  
			url: 'queries.php', 
			data: { sel_subject_section: $('#section').val()},
			dataType: 'json',
			success: function(data) {
...
function sel_specific_section(){
		var holder = new Array();
		$.ajax({  
			type: 'POST',  
			url: 'queries.php', 
			data: { sel_specific_section:  $('#teacher').val()},
			dataType: 'json',
			success: function(data) {
				$('#section').append("<option >Select Section</option>" );
				for (i=0;i<data.length;i++){
...
function sel_specific_subject(){
		...
		$.ajax({  
			type: 'POST',  
			url: 'queries.php', 
			data: { sel_specific_subject:  $('#teacher').val()},
			dataType: 'json',
			success: function(data) {
					$('#subject').empty();
				$('#subject').append("<option >Select Subject</option>" );
...
function send_sms(id)
{
		$.ajax({  
			type: 'POST',  
			url: 'send-sms.php', 
			data: {send_sms: id, subject: global_subject},
			dataType: 'json',
			success: function(data) {
				//alert(data)

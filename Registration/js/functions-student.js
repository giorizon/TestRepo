...
function add_student(id){
		...
		$.ajax({  
			type: 'POST',  
			url: 'queries.php', 
			data: { lrn: $('#lrn').val(), fname: $('#fname').val(),mname: $('#mname').val(), lname: $('#lname').val(), extname: $('#extname').val(), bday: $('#bday').val(), address: $('#address').val(), sex: $('#sex').val(), pppp:$('#pppp').val(), guardian_id: id},
			success: function(response) {
				add_to_section();
			}
});

  <nav class="navbar navbar-expand-lg fixed-top mynav" style = "border-bottom: 3px solid #008ae6;" id="mainNav">
 
    <div class="container">
      
		<a class="navbar-brand " href="../index.php"><img src= "img/home.png" style = "width: 35px;"></a>
	  
      <button class="navbar-toggler navbar-toggler-right" type="button" data-toggle="collapse" data-target="#navbarResponsive" aria-controls="navbarResponsive" aria-expanded="false" aria-label="Toggle navigation">
        Menu
        <i class="fas fa-bars"></i>
      </button>
       <div class="collapse navbar-collapse" id="navbarResponsive">
        <ul class="navbar-nav ml-auto">
          <li class="nav-item">
            <a class="nav-link js-scroll-trigger" href="add-teacher.php">Teacher</a>
          </li>
          <li class="nav-item">
            <a class="nav-link js-scroll-trigger" href="add-section.php">Section</a>
          </li>
          <li class="nav-item">
            <a class="nav-link js-scroll-trigger" href="add-class.php">Class</a>
          </li>
          <li class="nav-item">
            <a class="nav-link js-scroll-trigger" href="student.php">Student</a>
          </li>
           <li class="nav-item">
            <a class="nav-link js-scroll-trigger" href="attendance.php">Attendance</a>
          </li>
           <li class="nav-item">
            <a class="nav-link js-scroll-trigger" href="report.php">Report</a>
          </li>
           <li class="nav-item">
            <a class="nav-link js-scroll-trigger" href="../logout.php">Logout</a>
          </li>
        </ul>
      </div>
    </div>
  </nav>

